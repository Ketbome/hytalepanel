...
API ключ для інтеграції з репозиторієм модів [CurseForge](https://www.curseforge.com).

::: warning Важливо
API ключі CurseForge містять символи `$`, які інтерпретуються як змінні в shell/Docker. **Оберніть ключ в одинарні лапки** щоб уникнути проблем:
:::
...
Отримайте API ключ на [console.curseforge.com](https://console.curseforge.com).

Коли налаштовано, вмикає:

- Перегляд модів CurseForge у панелі
- Встановлення модів одним кліком з CurseForge
- Доступ до понад 2,600 модів Hytale

::: warning Обмеження поширення
Деякі моди CurseForge не дозволяють поширення через API. Ці моди показуватимуть попередження і повинні бути завантажені вручну з веб-сайту CurseForge.
:::
...
Повністю вимикає автентифікацію панелі. Використовуйте це, коли автентифікація здійснюється reverse proxy з SSO (Authentik, Authelia тощо).
...
::: warning Безпека
Вмикайте це лише за правильно налаштованим reverse proxy з автентифікацією. Панель буде повністю доступна без входу.
:::
...
# ===================
MODTALE_API_KEY=ваш-modtale-api-ключ
CURSEFORGE_API_KEY='$2a$10$ваш-curseforge-ключ'
