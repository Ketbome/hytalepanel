...
export async function downloadServerFiles(
  ...
  try {
    ...
    await docker.execCommand(
      "rm -f /opt/hytale/.download_attempted",
      30000,
      containerName,
    );
    ...
    const exec = await c.exec({
      Cmd: [
        "sh",
        "-c",
        `cd /opt/hytale && hytale-downloader ${channelFlag} -download-path ${zipPath} 2>&1`,
      ],
      AttachStdout: true,
      AttachStderr: true,
      Tty: true,
    ...
    stream.on("end", async () => {
      ...
      if (serverId) {
        socket.emit(
          "files",
          await files.checkServerFiles(serverId, containerName),
        );
        socket.emit(
          "downloader-auth",
          await files.checkAuth(serverId, containerName),
        );
      }
    });
